...
(function () {
  ...
  if (y.length > 1) {
    ...
    for (v in x) {
      ...
      // Thanks, @cmsalvado! :)
      if (!isNaN(parseFloat(wv)) && isFinite(wv)) wv = parseInt(wv);
  ...
  // Debug settings
  if (_settings.debug_noExtLoad == 1) headQueue.pushE_5grid = function (s) {};
  ...
  (function () {
    ...
    if (
      (doMobile &&
        (ww <= _settings.breakpoint_mobile ||
          sw <= _settings.breakpoint_mobile)) ||
      doMobileOnly
    ) {
    ...
    jQuery(function () {
      ...
      _nav_inner
        ...
        .bind("touchmove", function (e) {
          ...
          _nav_inner.scrollTop(
            _nav_inner_pos - e.originalEvent.touches[0].pageY,
          );
        });
      ...
      if (_settings.mobileUI_hideAddressBar == 1)
        _window.load(function () {
          if (_window.scrollTop() == 0) window.scrollTo(0, 1);
        });
    });
